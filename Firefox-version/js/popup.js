...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function saveSettings() {
    browser.runtime.sendMessage({ action: 'setSettings', settings: currentSettings });
  }
  ...
  browser.runtime.sendMessage({ action: 'getSettingsAndUpdates' }).then((response) => {
  if (response?.success) {
...
}).catch(err => console.warn('getSettingsAndUpdates failed:', err));

  browser.runtime.sendMessage({ action: 'getProfileData' }).then((response) => {
    if (response?.success) displayProfileData(response.data);
    else showError(response.error || t('profileFetchError'));
  }).catch(err => showError(t('profileFetchError') + ": " + (err.message || err)));

  unfollowerBtn.addEventListener('click', () => switchView(unfinderView));
  backToProfileBtn.addEventListener('click', () => {
    browser.runtime.sendMessage({ action: 'stopScan' });
  ...
  githubBtn.addEventListener('click', () => browser.runtime.sendMessage({ action: 'openGithub' }));
  updateBtn.addEventListener('click', () => browser.runtime.sendMessage({ action: 'openGithub' }));
  openHdBtn.addEventListener('click', () => browser.runtime.sendMessage({ action: 'openHdPhoto' }));
  downloadBtn.addEventListener('click', () => browser.runtime.sendMessage({ action: 'downloadPhoto' }));
  downloadJsonBtn.addEventListener('click', openExportModal);

  clearHistoryBtn.addEventListener('click', async () => {
    if (confirm(t('clearHistoryConfirm'))) {
      browser.runtime.sendMessage({ action: 'clearHistory' }).then(() => window.location.reload()).catch(()=>window.location.reload());
    }
  });
  ...
  startScanBtn.addEventListener('click', () => {
    browser.runtime.sendMessage({ action: 'startUnfollowScan' });
  ...
  unfollowSelectedBtn.addEventListener('click', () => {
    ...
    if (selectedUsers.length > 0) {
      browser.runtime.sendMessage({ action: 'unfollowSelected', users: selectedUsers });
  ...
  async function renderChart() {
    browser.storage.local.get('profileHistory').then(({ profileHistory }) => {
      if (!profileHistory) {
        chartSummary.innerHTML = t('noHistoryData');
                if (chartInstance) chartInstance.destroy();
        return;
      }
      browser.storage.local.get('cachedProfile').then(({ cachedProfile }) => {
  ...
  function loadScanTimings() {
    browser.storage.sync.get(['scanTimings']).then((data) => {
  ...
  document.getElementById('unfinderSettingsSaveBtn').addEventListener('click', () => {
    ...
    browser.storage.sync.set({ scanTimings: timings }).then(() => {
      browser.runtime.sendMessage({ action: 'setScanTimings', timings });
      unfinderSettingsPanel.style.display = 'none';
    }).catch(()=>{ unfinderSettingsPanel.style.display = 'none'; });
  });
